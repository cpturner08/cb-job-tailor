...
test -f AGENTS.md
test -f CLAUDE.md
test -f tests/fixtures/career-history.md
test -f tests/fixtures/resume.md
...
python3 scripts/check_resume.py --source tests/fixtures/career-history.md \
  tests/fixtures/resume.md

if grep -Eiq '(^|[^[:alnum:]_])PMP([^[:alnum:]_]|$)' tests/fixtures/resume.md; then
  echo "FAIL: unsupported PMP certification leaked into resume" >&2
  exit 1
fi

echo "PASS: tracked structure, privacy boundary, gap handling, and fact gate"
